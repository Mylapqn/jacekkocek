...
export class PolicyDatabase {
    static async setPolicy(name: string, value: number) {
        await connection.query(`INSERT INTO Policies (name, value) VALUES (\"${name}\",${value}) ON DUPLICATE KEY UPDATE Policies SET value=${value}`).catch(e => console.error("Set policy error: " + e));
    }
