...
export class KinoDatabase {
    ...
    static async loadEvents() {
        ...
        for (const eventData of events) {
            ...
            if (eventData["date_poll"]) eventOptions.filmPoll = Polls.Poll.list.find(p => p.id == eventData["date_poll"]);
...
export class PolicyDatabase {
    static async setPolicy(name: string, value: number) {
        await connection.query(`INSERT INTO Policies (name, value) VALUES (\"${name}\",${value}) ON DUPLICATE KEY UPDATE value=${value}`).catch(e => console.error("Set policy error: " + e));
    }
