...
export class DataObject {
    name: string;
    fields = new Array<DataField>();
    static translator = new Map<string, DataObject>();
    constructor(name: string, fields: Array<{ name: string, type: Datatype, isKey?: boolean }>) {
        this.name = name;
        DataObject.translator.set(name, this);
        for (const field of fields) {
            this.fields.push({ name: field.name, dbName: DataObject.convertToDbName(field.name), type: field.type, isKey: field.isKey });
        }
    }

    static convertToDbName(name: string) {
        let dbName = "";
        for (let i = 0; i < name.length; i++) {
            const letter = name[i];
            if (letter.toLocaleUpperCase() == letter) dbName += "_" + letter.toLocaleLowerCase(); else dbName += letter;
        }
        return dbName;
    }


    async push(obj: any, deep = false) {
        let fieldNames = "";
        let fieldValues = "";
        let updateString = "";
        for (let i = 0; i < this.fields.length; i++) {
            ...
            switch (field.type) {
                ...
                case "date":
                    fValue = `"${dateToSql(obj[field.name])}"`
                    break;

                default:
                    if(!deep) continue;
                    for (const link of obj[field.name]) {
                        await DataObject.translator.get(field.type.name).push(link);
                    }
                    continue;
            }
            ...
            fieldValues += fValue;
            updateString += field.dbName + " = " + fValue;
        ...
        return (await connection.query(`INSERT INTO ${this.name} (${fieldNames}) VALUES (${fieldValues}) ON DUPLICATE KEY UPDATE ${updateString}`)).insertId;
    }

    async pull(obj: any, deep = false) {
        let whereString = "";
        for (let i = 0; i < this.fields.length; i++) {
            const field = this.fields[i];
            ...
            whereString += field.dbName + " = " + fValue;
            if (i < this.fields.length - 1) {
                whereString += " AND ";
            }
        }
        ...
        for (const field of this.fields) {
            switch (field.type) {
                case "string":
                case "number":
                    obj[field.name] = data[field.dbName];
                    break;
                case "date":
                    obj[field.name] = new Date(data[field.dbName]);
                    break;

                default:
                    if(!deep) continue;
                    for (const link of obj[field.name]) {
                        await DataObject.translator.get(field.type.name).pull(link);
                    }
                    break;
            }
        }
